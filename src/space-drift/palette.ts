...
/** Pack a linear 0..1 color (optionally scaled by `mul`) into a 0xRRGGBB int. */
export const toHex = (c: Color, mul = 1): number => {
  const r = Math.max(0, Math.min(255, Math.round(c[0] * mul * 255)));
  const g = Math.max(0, Math.min(255, Math.round(c[1] * mul * 255)));
  const b = Math.max(0, Math.min(255, Math.round(c[2] * mul * 255)));
  return (r << 16) | (g << 8) | b;
};

/** The PICO-8 palette, in linear 0..1 colors. */
export const Pico8 = {
  black: rgb(0, 0, 0),
