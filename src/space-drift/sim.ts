...
export function enemyAiSystem(dt: number) {
  ...
  for (const enemy of enemies.raw) {
    ...
    if (e.state === 'engage' && ship) {
      // Pursuit: aim straight at the player and always thrust. Momentum + the
      // turn-rate cap make it close in, blow past, and loop back — it reaches
      // the player instead of holding distance.
